...
import { Plus, Pencil, Trash2, PenLine, Eye, MessageCircle, CalendarDays, Hash } from 'lucide-react';
...
const CHANNEL_IDENTITY: Record<string, { color: string; darkColor: string; icon: string }> = {
...
export function ContentPlanItemsList({
  ...
  const getChannelLabel = (value: string) =>
    MEDIA_CHANNELS_BY_TYPE.CONTENT.find((c) => c.value === value)?.label ?? value;
  const getCampaignTypeLabel = (value: string) =>
    CAMPAIGN_TYPES_BY_TYPE.CONTENT.find((c) => c.value === value)?.label ?? value;
  const getObjectiveLabel = (value: string) =>
    CAMPAIGN_OBJECTIVES_BY_TYPE.CONTENT.find((c) => c.value === value)?.label ?? value;

  const identity = (channel: string) => CHANNEL_IDENTITY[channel] ?? CHANNEL_IDENTITY.other;
        ...
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <p className="text-footnote text-muted-foreground mt-1">
              {totalPosts} bài viết · {items.length} kênh · {formatVND(allocatedBudget)} / {formatVND(totalBudget)}
            </p>
          </div>
          {isEditable && (
            <Button
            ...
            <div className="space-y-3">
              {items.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="group relative rounded-xl border border-border/50 hover:border-[#ff9f0a]/30 hover:shadow-sm transition-all overflow-hidden"
                  >
                    {/* Left accent bar */}
                    <div
                      ...
                      <div className="flex items-center gap-4 mt-3 pt-3 border-t border-border/30 flex-wrap">
                        {(item.targetReach ?? 0) > 0 && (
                          <div className="flex items-center gap-1.5 text-[11px]">
                            ...
                            <span className="tabular-nums font-medium">{item.targetReach}</span>
                        ...
                        {(item.targetLeads ?? 0) > 0 && (
                          <div className="flex items-center gap-1.5 text-[11px]">
                            ...
                            <span className="tabular-nums font-medium">
                              {(item.targetLeads! / 1000).toFixed(item.targetLeads! >= 1000 ? 0 : 1)}K
                            </span>
                        ...
                        {(item.targetCPL ?? 0) > 0 && (
                          <div className="flex items-center gap-1.5 text-[11px]">
                            ...
                            <span className="tabular-nums font-medium">{item.targetCPL}</span>
                            <span className="text-muted-foreground">bài/tuần</span>
                          </div>
                        )}
                      </div>
                    </div>
      ...
      {items.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="rounded-xl bg-[#ff9f0a]/5 dark:bg-[#ffd60a]/10 p-3.5 text-center">
            <p className="text-[22px] font-bold tabular-nums text-[#ff9f0a] dark:text-[#ffd60a]">{totalPosts}</p>
            <p className="text-caption text-muted-foreground mt-0.5">Tổng bài viết</p>
          </div>
          <div className="rounded-xl bg-[#ff9f0a]/5 dark:bg-[#ffd60a]/10 p-3.5 text-center">
            <p className="text-[22px] font-bold tabular-nums text-[#ff9f0a] dark:text-[#ffd60a]">{items.length}</p>
            <p className="text-caption text-muted-foreground mt-0.5">Kênh nội dung</p>
          </div>
          <div className="rounded-xl bg-[#ff9f0a]/5 dark:bg-[#ffd60a]/10 p-3.5 text-center">
            <p className="text-[22px] font-bold tabular-nums text-[#ff9f0a] dark:text-[#ffd60a]">
              {totalViews > 0 ? `${(totalViews / 1000).toFixed(0)}K` : '0'}
            </p>
            <p className="text-caption text-muted-foreground mt-0.5">Target Views</p>
          </div>
          <div className="rounded-xl bg-[#ff9f0a]/5 dark:bg-[#ffd60a]/10 p-3.5 text-center">
            <p className="text-[22px] font-bold tabular-nums text-[#ff9f0a] dark:text-[#ffd60a]">
              {items.length > 0
                ? (items.reduce((s, i) => s + (i.targetCPL ?? 0), 0) / items.length).toFixed(1)
                : '0'}
            </p>
            <p className="text-caption text-muted-foreground mt-0.5">TB bài/tuần</p>
          </div>
        </div>
      )}
      ...
      <AlertDialog open={!!deletingItemId} onOpenChange={() => setDeletingItemId(null)}>
        <AlertDialogContent className="rounded-2xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Bạn có chắc muốn xóa nội dung này khỏi kế hoạch? Hành động này không thể hoàn tác.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
